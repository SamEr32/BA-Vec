    ...
    for i, shape_group in enumerate(shape_groups):
        def add_color(shape_color, name):
            ...
            if isinstance(shape_color, pydiffvg.RadialGradient):
                ...
                cx, cy = lg.center[0].item(), lg.center[1].item()
                rx, ry = lg.radius[0].item(), lg.radius[1].item()
                ...
                if abs(rx - ry) < 1e-6:
                    ...
                    # 是椭圆，统一 r=1，使用 gradientTransform 缩放椭圆半径
                    color.set('r', '1.0')

                    # 构造仿射矩阵 matrix(a b c d e f)
                    a = rx
                    b = 0
                    c = 0
                    d = ry
                    e = cx * (1 - rx)
                    f = cy * (1 - ry)
                    color.set('gradientTransform', f'matrix({a} {b} {c} {d} {e} {f})')
